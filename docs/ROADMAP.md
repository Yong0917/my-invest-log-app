...
### Phase 2: 실제 기능 연동 — MVP 완성 (2주 — 2026-03-27 ~ 2026-04-10)
...
- [ ] Supabase `portfolios` 테이블 마이그레이션 SQL 작성 및 적용 | 담당: 풀스택 | 예상: 0.5d | 우선순위: 🔴높음
  ...
  - 의존성: 이후 모든 DB·API 태스크의 선행 조건
- [ ] Supabase RLS 정책 적용 (SELECT/INSERT/UPDATE/DELETE 모두 `auth.uid() = user_id` 조건) | 담당: 풀스택 | 예상: 0.5d | 우선순위: 🔴높음
  - 의존성: 테이블 마이그레이션 완료 후
- [ ] `lib/supabase/types.ts` 또는 Supabase CLI로 TypeScript 타입 자동 생성 | 담당: 풀스택 | 예상: 0.5d | 우선순위: 🔴높음

**인증 연동 (F010)**

- [ ] 기존 `/auth/login` 페이지에 Supabase Auth `signInWithPassword` Server Action 연결 | 담당: 풀스택 | 예상: 1d | 우선순위: 🔴높음
  - 로그인 성공 시 `/protected/dashboard` 리다이렉트
  - 실패 시 에러 메시지 표시
  - 의존성: DB 마이그레이션 완료 후
- [ ] 로그아웃 버튼 구현 (`signOut` → `/auth/login` 리다이렉트) | 담당: 풀스택 | 예상: 0.5d | 우선순위: 🔴높음
- [ ] `proxy.ts` 미인증 접근 차단 검증 (직접 URL 접근 시 `/auth/login` 리다이렉트 확인) | 담당: 풀스택 | 예상: 0.5d | 우선순위: 🔴높음

**포트폴리오 CRUD — Supabase 연동**

- [ ] `app/actions/portfolio.ts` Server Actions 파일 생성 | 담당: 풀스택 | 예상: 0.5d | 우선순위: 🔴높음
  - 의존성: RLS 정책 적용, 타입 생성 완료 후
- [ ] `getPortfolios(userId)` Server Action 구현 (Supabase SELECT) | 담당: 풀스택 | 예상: 0.5d | 우선순위: 🔴높음
- [ ] `createPortfolio(data)` Server Action 구현 (Supabase INSERT, UNIQUE 충돌 에러 처리) | 담당: 풀스택 | 예상: 1d | 우선순위: 🔴높음
- [ ] `updatePortfolio(id, data)` Server Action 구현 (Supabase UPDATE) | 담당: 풀스택 | 예상: 0.5d | 우선순위: 🟡중간
- [ ] `deletePortfolio(id)` Server Action 구현 (Supabase DELETE) | 담당: 풀스택 | 예상: 0.5d | 우선순위: 🟡중간
- [ ] 대시보드 페이지를 Server Component로 전환하여 `getPortfolios` 호출 및 초기 데이터 전달 | 담당: 풀스택 | 예상: 1d | 우선순위: 🔴높음

**현재가 조회 — Yahoo Finance API 연동 (F003)**

- [ ] `app/api/stock-price/route.ts` Route Handler 구현 | 담당: 풀스택 | 예상: 1d | 우선순위: 🔴높음
  - `GET /api/stock-price?ticker=005930.KS` 형태
  - Yahoo Finance `https://query1.finance.yahoo.com/v8/finance/chart/{ticker}` 호출
  - 응답에서 현재가(`regularMarketPrice`) 추출 반환
  - 의존성: DB 마이그레이션 완료 후 독립 구현 가능
- [ ] API 에러 처리: 잘못된 티커, Yahoo Finance 타임아웃, 장 마감 시 처리 | 담당: 풀스택 | 예상: 0.5d | 우선순위: 🟡중간
- [ ] 대시보드 Client Component에서 종목 목록 로드 후 현재가 병렬 조회 (`Promise.all`) | 담당: 풀스택 | 예상: 1d | 우선순위: 🔴높음
  - 조회 중 로딩 스피너 표시, 실패 시 "-" 표시

**종목명 자동 조회 (F011)**

- [ ] 주식 등록 모달 종목코드 입력 후 Yahoo Finance API로 종목명 자동 조회 기능 구현 | 담당: 풀스택 | 예상: 1d | 우선순위: 🟡중간
  - "조회" 버튼 클릭 시 `/api/stock-price?ticker=` 호출 → 종목명 필드 자동 입력
  - 의존성: Route Handler 구현 완료 후

**수익률 통합 (F004)**

- [ ] Phase 1에서 검증한 `lib/calculate.ts` 함수를 실제 Supabase 데이터 + Yahoo Finance 현재가에 연결 | 담당: 풀스택 | 예상: 0.5d | 우선순위: 🔴높음
- [ ] 대시보드 Mock 데이터 → 실제 데이터 전환 및 전체 흐름 통합 테스트 | 담당: 풀스택 | 예상: 1d | 우선순위: 🔴높음
